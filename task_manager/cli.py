...
Examples:
  ...
  task-manager search "urgent"            # Search tasks
        ...
        add_parser.add_argument('-c', '--category', default='general', help='Task category')
        ...
        list_parser.add_argument('-s', '--status', choices=['todo', 'in_progress', 'done', 'cancelled'],
        ...
        list_parser.add_argument('--due-soon', type=int, metavar='DAYS',
                            help='Show tasks due within N days')
        ...
        update_parser.add_argument('-p', '--priority', choices=['low', 'medium', 'high', 'urgent'],
                               help='New priority')
        ...
        progress_parser = subparsers.add_parser('progress', help='Mark task as in progress')
        ...
        export_parser.add_argument('-f', '--format', choices=['json', 'csv', 'markdown'],
    ...
    def update_task(self, args) -> None:
        ...
        if success:
            self.console.print(f"[green]✅ Task {args.task_id} updated successfully[/green]")
        else:
            self.console.print(f"[red]Task with ID {args.task_id} not found.[/red]")
    
    def mark_done(self, args) -> None:
        """Mark task as done."""
    ...
    def mark_in_progress(self, args) -> None:
        """Mark task as in progress."""
        if self.manager.mark_task_in_progress(args.task_id):
            self.console.print(f"[yellow]🔄 Task {args.task_id} marked as in progress[/yellow]")
        else:
            self.console.print(f"[red]Task with ID {args.task_id} not found.[/red]")
    
    def mark_cancelled(self, args) -> None:
    ...
    def search_tasks(self, args) -> None:
        ...
        if not tasks:
            self.console.print(f"[yellow]No tasks found matching '{args.query}'[/yellow]")
            return
        
        self.console.print(f"[cyan]Found {len(tasks)} tasks matching '{args.query}':[/cyan]")
        self.show_tasks_table(tasks)
...
📊 Completion Rate: {stats['completion_rate']}%
    ...
    def export_tasks(self, args) -> None:
    ...
    def show_dashboard(self) -> None:
        ...
        # Show recent tasks
        recent_tasks = sorted(self.manager.get_all_tasks(), 
                         key=lambda t: t.created_at, reverse=True)[:5]
    ...
    def show_tasks_table(self, tasks: List, show_details: bool = True) -> None:
    ...
    def show_task_details(self, task) -> None:
        """Show detailed task information."""
        status_text = {
            TaskStatus.TODO: "⏳ Todo",
            TaskStatus.IN_PROGRESS: "🔄 In Progress",
...
[bold]Updated:[/bold] {task.updated_at.strftime('%Y-%m-%d %H:%M')}
        ...
        panel = Panel(
            details,
            title=f"[bold blue]Task Details[/bold blue]",
            border_style="blue"
        )
        self.console.print(panel)
    
    def show_task_simple(self, task) -> None:
